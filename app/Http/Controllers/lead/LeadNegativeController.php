...
class LeadNegativeController extends Controller {
    ...
    public function index() {
        ...
        if (config('constants.roles.administrator') == Auth::user()->role_id) {
          //  $leads = Generallead::where('conversionstatus_id', '=', 2)->where('branch_id', '<>', NULL)->get();
            $leads = Generallead::where('conversionstatus_id', '=', 2)->get();
        } else if (config('constants.roles.branchmanager') == Auth::user()->role_id) {
            $leads = Generallead::where('conversionstatus_id', '=', 2)->where('branch_id', '=', Auth::user()->branch_id)->get();
            ...
            $leads = Generallead::where('conversionstatus_id', '=', 2)
        ...
        return view('lead.leads_negative')->with('data', $data);
    }
    ...
    public function edit($id) {
        ...
        try {
            LogData::log('Edit Negative Lead Start', 'edit', 'Start');
            $branch = Branch::all();
            ...
            LogData::logError('Edit Negative Lead Error', 'edit', $ex->getMessage());
    ...
    public function followUp(Request $data) {
        ...
        try {
            LogData::log('Follow up Negative Lead Start', 'followUp', 'Start');
            ...
            LogData::logError('Follow up Negative Lead Error', 'followUp', $ex->getMessage());
            return array(
                'success' => false,
                'error' => $ex->getMessage(),
            );
